<h2>
	...
	<?php if($this->user) : ?>
	<?php if($favorite): ?><a href="<?= site_url("podcasts/removeFavorite/".$podcast->id) ?>" class="btn btn-danger margin-right-10"> <i></i>PodCast Favori</a> <!--Coeur plein. action over : coeur barré-->
	<?php else :?><a href="<?= site_url("podcasts/addFavorite/".$podcast->id) ?>" class="btn btn-danger margin-right-10"> Ajouter à mes Favoris</a> <!--Coeur vide. action over : coeur plein.-->
    <?php endif ?>
